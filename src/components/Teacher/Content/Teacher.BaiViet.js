...
const TeacherBaiViet = () => {
  ...
  const fetchChuyenNganhData = async () => {
    try {
      ...
      // Updates the state with the list of chuyên ngành
    } catch (error) {
      console.error("Error fetching data:", error);
      // Logs an error message to the console if there is a problem fetching the data
    }
  };
  ...
  const handleModalOk = async () => {
    try {
        ...
        // Validate and append the file if it exists
      ...
      // After success, close modal and refresh data
